    ...
    <div className="stats-grid">
      {STAT_META.map((s, i) => {
        ...
        return (
          ...
          >
            {active && (
              <motion.div layoutId={`strip-${s.key}`}
                style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: s.color, borderRadius: "var(--radius) var(--radius) 0 0" }}
              />
            )}
            ...
            <div style={{ fontSize: 24, fontWeight: 800, color: s.color, lineHeight: 1 }}>
              {fmt(s.key, stats[s.key])}
            </div>
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 5, fontWeight: 500 }}>
              {s.label}
            </div>
            {active && (
              <div style={{ fontSize: 9, color: s.color, marginTop: 3, fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase" }}>
                Filtering ✓
              </div>
            )}
          </motion.div>
